...
function App() {
    ...
    const [filteringImage, setFilteringImage] = useState<{ decade: string; url: string } | null>(null);
    ...
    const handleDownloadIndividualImage = (decade: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            setFilteringImage({ decade, url: image.url });
        }
    };

    const handleDownloadWithFilter = async (decade: string, imageUrl: string, filterCss: string) => {
        // Simple download for 'original'
        if (filterCss === 'none') {
        ...
        try {
            ...
            link.download = `past-forward-${decade}-filtered.jpg`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
    
        } catch (error) {
            console.error("Failed to apply filter and download image:", error);
            alert("Sorry, there was an error applying the filter. The image may not be downloadable. Please try again.");
        }
    };
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            {filteringImage && (
                <FilterModal
                    ...
                    onDownload={handleDownloadWithFilter}
                />
            )}
        </main>
    );
